    ...
    <nav className="fixed top-0 left-0 w-full z-50 px-6 py-4">
      <div className="max-w-7xl mx-auto flex items-center justify-between glass rounded-2xl px-6 py-3">
        ...
        <div className="hidden md:flex items-center gap-8">
            ...
            link.href.startsWith("/#") ? (
              ...
              <Link
                ...
                className={`text-sm font-medium transition-colors ${
                  location.pathname === link.href ? "text-ink" : "text-ink/60 hover:text-ink"
                }`}
              >
                {link.name}
              </Link>
          ...
          <Link to="/start-project" className="bg-ink text-bg px-5 py-2 rounded-xl text-sm font-bold hover:bg-accent hover:text-white transition-colors flex items-center gap-2 group">
            Start Project
